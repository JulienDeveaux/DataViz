...
void draw() {
  ...
  if (error) {
    fill(255, 0, 0);
    textSize(20);
    text("Donnée innexistante ou erronée", width/5, height/2);
    textSize(10);
  } else {
...
void setupClassements() {
  ...
  String[] toLoad = chargerClassements(Integer.parseInt(date[0]),
    Integer.parseInt(date[1]),
    Integer.parseInt(date[2]));
  if (toLoad.length != nequipes) {
    error = true;
  } else {
    error = false;
    classements = new ListeClassements(toLoad);
